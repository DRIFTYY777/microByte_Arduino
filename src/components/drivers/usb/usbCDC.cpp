...
void usbCDC::usbEventCallback(void *arg, esp_event_base_t event_base, int32_t event_id, void *event_data) {
...
void usbCDC::init() {
    ...
    USBSerial.begin(); // Initialize USB CDC with default baud rate
    USB.begin();  // Initialize USB stack
}

int usbCDC::usbSerialAvailable() {
    return USBSerial.available();
}

uint32_t usbCDC::baudRate() {  // Add usbCDC:: scope resolution
...
size_t usbCDC::read(uint8_t *buffer, size_t size) {
    return USBSerial.read(buffer, size);
}

size_t usbCDC::write(const uint8_t *buffer, size_t size) {
    return USBSerial.write(buffer, size);
}
