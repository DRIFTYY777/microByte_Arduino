...
#include <components/drivers/display/ST7789/ST7789_driver.h>
...
uint16_t DisplayHAL::getPixelGBC(const uint16_t *bufs, uint16_t x, uint16_t y, uint16_t w2, uint16_t h2)
{
...
uint8_t DisplayHAL::getPixelSMS(const uint8_t *bufs, uint16_t x, uint16_t y, uint16_t w2, uint16_t h2, bool GAME_GEAR)
{
...
uint8_t DisplayHAL::getPixelNES(const uint8_t *bufs, uint16_t x, uint16_t y, uint16_t w2, uint16_t h2)
{
    int x_diff, y_diff, xv, yv, red, green, blue, col, a, b, c, d, index;
...
bool DisplayHAL::display_HAL_init(void)
{
    return ST7789_init(&display);
}

void DisplayHAL::display_HAL_clear()
{
    ST7789_fill_area(&display, WHITE, 0, 0, display.display_width, display.display_height);
}

void DisplayHAL::display_HAL_flush(lv_disp_drv_t *drv, const lv_area_t *area, lv_color_t *color_map)
{
    ...
    display.current_buffer = (st7789_color_t *)color_map;
...
void DisplayHAL::display_HAL_gb_frame(const uint16_t *data)
{
...
void DisplayHAL::display_HAL_NES_frame(const uint8_t *data)
{
    uint16_t calc_line = 0;
    uint16_t sending_line = 0;
...
void DisplayHAL::display_HAL_SMS_frame(const uint8_t *data, uint16_t color[], bool GAMEGEAR)
{
    uint16_t sending_line = 0;
    uint16_t calc_line = 0;
...
uint16_t *DisplayHAL::display_HAL_get_buffer()
{
    return display.current_buffer;
}

size_t DisplayHAL::display_HAL_get_buffer_size()
{
    return display.buffer_size;
}

void DisplayHAL::display_HAL_boot_frame(uint16_t *buffer)
{
...
void DisplayHAL::display_HAL_change_endian()
{
