...
void display_HAL_gb_frame(const uint16_t *data)
{
    ...
    if (data == NULL)
    {
        for (uint16_t y = 0; y < SCR_HEIGHT; y++)
        {
            ...
            sending_line = calc_line;
            calc_line = (calc_line == 1) ? 0 : 1;
            ST7789_write_lines2(&display, y, 0, SCR_WIDTH, line[sending_line], 1);
        }
    }
    else
    {
        ...
        for (int y = 0; y < outputHeight; y += LINE_COUNT)
        {
            ...
            calc_line = (calc_line == 1) ? 0 : 1;
            // ST7789_swap_buffers(&display);
            ST7789_write_lines2(&display, y, xpos, outputWidth, line[sending_line], LINE_COUNT);
        }
    }
}

uint16_t getPixelGBC(const uint16_t *bufs, uint16_t x, uint16_t y, uint16_t w2, uint16_t h2)
{
    int x_diff, y_diff, xv, yv, red, green, blue, col, a, b, c, d, index;
