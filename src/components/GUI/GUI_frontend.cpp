...
#include <components/GUI/icons/emulators_icon.h>
#include <components/GUI/icons/ext_application_icon.h>
#include <components/GUI/icons/settings_icon.h>
#include <components/GUI/icons/nes_icon.h>
#include <components/GUI/icons/snes_icon.h>
#include <components/GUI/icons/gameboycolor_icon.h>
#include <components/GUI/icons/gameboy_icon.h>
#include <components/GUI/icons/sega_icon.h>
#include <components/GUI/icons/GG_icon.h>
#include <components/GUI/icons/nogame_icon.h>
...
#include <components/drivers/display/backlight_ctrl/backlight_ctrl.h>
...
void GUI_FRONTEND::GUI_frontend(void)
{
    ...
    uint8_t battery_aux = battery.battery_get_percentage();
    // char *battery_level = malloc(4);
    char *battery_level = (char *)malloc(4);
    ...
    if (!sd_card.sd_mounted())
        lv_obj_set_hidden(SD_label, true);
    ...
    */
    ...
    uint8_t theme_color = sys_manager.system_get_config(SYS_GUI_COLOR);
...
void GUI_FRONTEND::async_battery_alert()
{
...
bool GUI_FRONTEND::user_input_task(lv_indev_drv_t *indev_drv, lv_indev_data_t *data)
{
...
void GUI_FRONTEND::battery_status_task(lv_task_t *task)
{
...
void GUI_FRONTEND::emulators_menu(lv_obj_t *parent)
{
...
void GUI_FRONTEND::emulators_list_cb(lv_obj_t *parent, lv_event_t e)
{
...
void GUI_FRONTEND::game_execute_cb(lv_obj_t *parent, lv_event_t e)
{
    if (e == LV_EVENT_CLICKED)
    {
        struct SYSTEM_MODE emulator;
        ...
        {
            sd_card.sd_sav_remove((char *)lv_msgbox_get_text(mbox_game_options), emulator_selected);
...
void GUI_FRONTEND::game_menu_cb(lv_obj_t *parent, lv_event_t e)
{
    if (e == LV_EVENT_CLICKED)
    {
        ...
        if (sd_card.sd_sav_exist((char *)lv_list_get_btn_text(parent), emulator_selected))
        {
...
void GUI_FRONTEND::msgbox_no_game_cb(lv_obj_t *msgbox, lv_event_t e)
{
...
void GUI_FRONTEND::game_list_cb(lv_obj_t *parent, lv_event_t e)
{
...
void GUI_FRONTEND::on_game_menu()
{
...
void GUI_FRONTEND::slider_volume_cb(lv_obj_t *slider, lv_event_t e)
{
    if (e == LV_EVENT_VALUE_CHANGED)
    {
        ESP_LOGI(TAG, "Volumen set: %i", lv_slider_get_value(slider));
        sound.audio_volume_set(lv_slider_get_value(slider));
    }
    else if (e == LV_EVENT_CANCEL)
    {
...
void GUI_FRONTEND::slider_brightness_cb(lv_obj_t *slider, lv_event_t e)
{
    if (e == LV_EVENT_VALUE_CHANGED)
    {
        ESP_LOGI(TAG, "Brightness set: %i", lv_slider_get_value(slider));
        backlight.backlight_set(lv_slider_get_value(slider));
    }
    else if (e == LV_EVENT_CANCEL)
    {
...
void GUI_FRONTEND::list_game_menu_cb(lv_obj_t *parent, lv_event_t e)
{
    if (e == LV_EVENT_CLICKED)
    {
        struct SYSTEM_MODE emulator;
        ...
        else if (strcmp(lv_list_get_btn_text(parent), "Volume") == 0)
        {
            ...
            // Get volumen
            uint8_t volume = sound.audio_volume_get();
        ...
        else if (strcmp(lv_list_get_btn_text(parent), "Brightness") == 0)
        {
            ...
            uint8_t brightness_level = backlight.backlight_get();
...
void GUI_FRONTEND::external_app_menu(lv_obj_t *parent)
{
...
void GUI_FRONTEND::external_app_cb(lv_obj_t *parent, lv_event_t e)
{
    if (e == LV_EVENT_CLICKED)
    {
...
void GUI_FRONTEND::app_execute_cb(lv_obj_t *parent, lv_event_t e)
{
    if (e == LV_EVENT_CLICKED)
    {
...
void GUI_FRONTEND::config_menu(lv_obj_t *parent)
{
...
void GUI_FRONTEND::configuration_cb(lv_obj_t *parent, lv_event_t e)
{
    if (e == LV_EVENT_CLICKED)
    {
        ...
        lv_obj_set_event_cb(list_btn, config_option_cb);
        if (sys_manager.system_get_config(SYS_STATE_SAV_BTN) != 1)
            list_btn = lv_list_add_btn(list_config, LV_SYMBOL_SAVE, "Enable Button State Save");
        else if (sys_manager.system_get_config(SYS_STATE_SAV_BTN))
...
void GUI_FRONTEND::config_option_cb(lv_obj_t *parent, lv_event_t e)
{
    if (e == LV_EVENT_CLICKED)
    {
        ...
        else if (strcmp(lv_list_get_btn_text(parent), "Update firmware") == 0)
        {
            ...
            // Get the game list of each console.
            char *fw_update_name[100];
            uint8_t fw_update_num = sd_card.sd_app_list(fw_update_name, true);
        ...
        else if (strcmp(lv_list_get_btn_text(parent), "GUI Color Mode") == 0)
        {
            ...
            {
                sys_manager.system_save_config(SYS_GUI_COLOR, 1);
                GUI_theme_color(1);
            }
            else
            {
                sys_manager.system_save_config(SYS_GUI_COLOR, 0);
                GUI_theme_color(0);
            }
        }
        else if (strcmp(lv_list_get_btn_text(parent), "Volume") == 0)
        ...
        else if (strcmp(lv_list_get_btn_text(parent), "SD card Status") == 0)
        {
            ...
            if (sd_card.sd_mounted())
            {
                ...
                sprintf(spec_text, "\u2022 SD Name: %s\n\u2022 Size: %i MB \n\u2022 Speed: %i Khz\n\u2022 Type: %s", sd_card_info.card_name, sd_card_info.card_size, sd_card_info.card_speed,
...
void GUI_FRONTEND::mbox_config_cb(lv_obj_t *parent, lv_event_t e)
{
    if (e == LV_EVENT_CANCEL)
    {
        lv_obj_del(parent);
...
void GUI_FRONTEND::fw_update_cb(lv_obj_t *parent, lv_event_t e)
{
    if (e == LV_EVENT_CLICKED)
    {
...
// void GUI_FRONTEND::GUI_theme_color(uint8_t color_selected)
// {
//     if (color_selected)
//         GUI_THEME = LV_THEME_MATERIAL_FLAG_DARK;
//     else
//         GUI_THEME = LV_THEME_MATERIAL_FLAG_LIGHT;
...
void GUI_FRONTEND::GUI_theme_color(uint8_t color_selected)
{
    ...
    // Set the theme to the current display
