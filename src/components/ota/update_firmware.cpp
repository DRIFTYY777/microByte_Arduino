...
void UPDATE_FIRMWARE::update_check()
{
...
int UPDATE_FIRMWARE::update_init(char *fw_name)
{
    ...
    ESP_LOGI(TAG, "Found available partition subtype %d at offset 0x%x with label %s", update_partition->subtype, update_partition->address, update_partition->label);
