...
void FirmwareEvientHandler(lv_event_t *e)
{
...
void createOTAScreen()
{
    ...
    // title bar
    notificationBar(new_screen, false, "Firmware Update");
    ...
    char *app_list[100];
    uint8_t app_num = sd_card.sd_app_list(app_list, true);
    ...
    if (app_num > 0)
    {
        for (uint8_t i = 0; i < app_num; i++)
        {
            ...
            lv_obj_add_event_cb(btn, FirmwareEvientHandler, LV_EVENT_CLICKED, NULL);
