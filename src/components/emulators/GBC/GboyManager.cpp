...
void gnuboy_start()
{
    ...
    button_ss_gb = sys_manager.system_get_config(SYS_STATE_SAV_BTN);

    // Execute emulator tasks.
    xTaskCreatePinnedToCore(&videoTask, "videoTask", 1024 * 5, NULL, 1, &videoTask_handler, 0);
...
bool gnuboy_execute_game(const char *name, uint8_t console, bool load)
{
    ...
    game_name = (char *)malloc(strlen(name));
...
static void gnuBoyTask(void *arg)
{
    ...
    displayBuffer[0] = (uint16_t *)heap_caps_malloc(160 * 144 * 2, MALLOC_CAP_8BIT | MALLOC_CAP_DMA);
    displayBuffer[1] = (uint16_t *)heap_caps_malloc(160 * 144 * 2, MALLOC_CAP_8BIT | MALLOC_CAP_DMA);

    if (displayBuffer[0] == NULL)
    {
        ESP_LOGW(TAG, "DisplayBuffer[0] not enough DMA memory for allocate. \n Allocating on regular memory.");
        displayBuffer[0] = (uint16_t *)malloc(160 * 144 * 2);
    ...
    if (displayBuffer[1] == NULL)
    {
        ...
        displayBuffer[1] = (uint16_t *)malloc(160 * 144 * 2);
    ...
    fb.indexed = 0;
    fb.ptr = (unsigned char *)framebuffer;
    fb.enabled = 1;
    fb.dirty = 0;
    ...
    audioBuffer[0] = (unsigned char *)heap_caps_malloc(AUDIO_BUFFER_SIZE, MALLOC_CAP_8BIT | MALLOC_CAP_DMA);
    audioBuffer[1] = (unsigned char *)heap_caps_malloc(AUDIO_BUFFER_SIZE, MALLOC_CAP_8BIT | MALLOC_CAP_DMA);

    if (audioBuffer[0] == NULL)
    {
        ESP_LOGW(TAG, "audioBuffer[0] not enough DMA memory for allocate. \n Allocating on regular memory.");
        audioBuffer[0] = (unsigned char *)malloc(AUDIO_BUFFER_SIZE);
    ...
    if (audioBuffer[1] == NULL)
    {
        ...
        audioBuffer[1] = (unsigned char *)malloc(AUDIO_BUFFER_SIZE);
    ...
    if (load_save_game)
    {
        if (!gbc_state_load(game_name, console_use))
            ESP_LOGW(TAG, "Error loading save game, starting new save game.");
    }
    ...
    while (1)
    {
        ...
        // frequency is board_build.f_cpu = 240000000L
        float freq = 240000000.0f;

        if (actualFrameCount == 60)
        {
            float seconds = totalElapsedTime / (freq * 1000000.0f);
...
static void run_to_vblank()
{
    ...
    while (R_LY > 0 && R_LY < 144)
        emu_step(); // Step through visible line scanning phase
    ...
    while (R_LY > 0)
        emu_step(); // Step through vblank phase
}
...
static void input_set()
{
    ...
    pad_set(PAD_DOWN, !((inputs_value >> 1) & 0x01));
    pad_set(PAD_LEFT, !((inputs_value >> 3) & 0x01));
    pad_set(PAD_UP, !((inputs_value >> 0) & 0x01));
    pad_set(PAD_RIGHT, !((inputs_value >> 2) & 0x01));

    pad_set(PAD_B, !((inputs_value >> 10) & 0x01));
    pad_set(PAD_A, !((inputs_value >> 9) & 0x01));

    pad_set(PAD_START, !((inputs_value >> 6) & 0x01));
    pad_set(PAD_SELECT, !((inputs_value >> 7) & 0x01));

    // Special function to instant load/save progress pushing x and y buttons
    if (!((inputs_value >> 11) & 0x01) && button_ss_gb)
        gbc_state_load(game_name, console_use);
    if (!((inputs_value >> 12) & 0x01) && button_ss_gb)
        gnuboy_save();
